...
function lock() {
    ...
    local vram_total
    vram_total=$(nvidia-smi --query-gpu=memory.total --format=csv,noheader,nounits 2>/dev/null | grep -v -e Failed -e library | grep -oP '^\d+$' || : | head -1)
    vram_total=${vram_total:-8188}
