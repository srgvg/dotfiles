_starship() {
    ...
    case "${cmd}" in
        ...
        starship__module)
            ...
            ;;
        starship__preset)
            opts="-o -l -h --output --list --help bracketed-segments plain-text-symbols pure-preset no-nerd-font tokyo-night nerd-font-symbols pastel-powerline no-runtime-versions no-empty-icons"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
